 ...
 *********************************************/
/* hacked up with lots of wavetables in progmem by Josh Brandt, 6 October 2008 */
...
const uint8_t waveTable[] PROGMEM = {
  // sine
...
uint32_t sample;     // final sample that goes to the DAC    

// variables for oscillator with no modulation
...
ISR(TIMER2_COMPA_vect) {
  ...
  PORTB &= ~(1 << 2); // Frame sync low for SPI (making it low here so that we can measure lenght of interrupt with scope)

  phaseDeltaModulator = (frequency * harmonicity) >> 6;   // calculate modulator frequency for a given harmonicity
  phaseDeltaCarrier = frequency;                          // this is just pitch (although not in Hz)
  ...
  memcpy_P(&modulator, &waveTable[modulatorWaveForm * 256 + indexModulator], 1);
  ...
  memcpy_P(&carrier, &waveTable[waveForm * 256 + indexCarrier], 1);
  ...
  //Serial.print("M = ");
  //Serial.println(SPCR & (1 << MSTR), DEC);
